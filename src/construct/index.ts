...
export type ConstructMiddleware<T extends ConstructTypes> = (
	scope: Construct,
	id: string
) => GenericExtend<T>;
...
export const createConstructs = <
	T extends ConstructTypes,
	K extends Constructs<T>
>({
	type,
	constructs
		...
		_constructs[name] = (scope, id): GenericExtend<T> => {
			...
			if (type.name === NodejsFunction.name) {
				const _entry = createFileNameHandler(
					resolve(
						`${getParentDirectory(getCallerFile())}`,
						'..',
						'constructs',
						'lambda'
					),
